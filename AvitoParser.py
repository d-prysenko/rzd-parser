...
class AvitoParser:
    baseUrl = 'https://www.avito.ru/voronezh?q=iphone+xs'
    # baseUrl = 'http:// 0.0.0.0:8080'
    client = Client()

    def getAds(self, search_str, sort=Sort.date):
        url = self.baseUrl + '&s=' + str(sort.value)
        print('get ' + url)

        response = self.client.get(url)

        with open('resp.html', 'w') as fresp:
            fresp.write(response.content.decode('utf-8'))
